...
# 					print str(env.now) + ': New transfer from customer ' + str(self.id)
				...
				if i.cause[0] == 'portAllocated':
					...
					yield env.timeout(i.cause[2]) # WDM configuration delay
					self.updateRate(R/len(self.requests))
...
class Transfer:
	...
	def __init__(self, env, id, timer, customer, size, output, delayThreshold, rate, portId):
		...
		if self.flowRate != 0:
			...
			self.startTime = self.requestTime
		else:
...
# 				pdb.set_trace()
				if self.flowRate == 0:
					...
					if self.startTime - self.requestTime > self.delayThreshold:
						self.flag = True
					assert i.cause[1] != 0
# 					if self.startTime - self.requestTime > 0.001:
...
# 			pdb.set_trace()
		with open(self.output, 'a') as f:	
			writer = csv.writer(f)
			writer.writerow( (self.id, self.size, float("{0:.3f}".format(self.startTime - self.requestTime)), float("{0:.3f}".format(env.now - self.requestTime))) )
