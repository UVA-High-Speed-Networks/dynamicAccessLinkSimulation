...
class AccessLink:
	...
	def __init__(self, env, pipeToController, ip100GProc, lossRate, id, arrivalRate, output):
		...
		self.countTotal = 0
		self.countDelay = 0
		self.ip10G = IP.IPSimulator(env, G.Rl, lossRate)
...
# 				print self.startTime - self.requestTime
		...
		with open(self.output, 'a') as f:	
			writer = csv.writer(f)
			writer.writerow( (self.id, float("{0:.3f}".format(self.size)), float("{0:.3f}".format(self.startTime - self.requestTime)), float("{0:.3f}".format(env.now + fileOpenOverhead - self.requestTime)), path) )
